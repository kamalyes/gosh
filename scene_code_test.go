 ...
 * @LastEditTime: 2024-11-16 17:16:15
 * @FilePath: \gosh\scene_code_test.go
...
func TestSetAndGetSceneCode(t *testing.T) {
	// 定义一个自定义状态码
	const customCode SceneCode = 2001
	customMsg := "Custom Success"

	// 设置自定义状态码
	SetSceneCode(customCode, customMsg)

	// 获取自定义状态码对应的消息
	got := GetSceneCodeText(customCode)
	if got != customMsg {
...
func TestInitSceneCodes(t *testing.T) {
	// 定义自定义状态码和消息
	customCodes := map[SceneCode]string{
		200:  "Custom Success",
		2002: "Custom Error",
	...
	// 初始化自定义状态码
	InitSceneCodes(customCodes)

	// 验证自定义状态码的消息
	for code, expectedMsg := range customCodes {
		got := GetSceneCodeText(code)
		if got != expectedMsg {
...
func TestGetUnknownSceneCode(t *testing.T) {
	unknownCode := SceneCode(9999) // 一个未定义的状态码

	// 获取未知状态码的消息
	got := GetSceneCodeText(unknownCode)
	if got != "" {
