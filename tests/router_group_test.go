 ...
 * @FilePath: \gosh\tests\router_group_test.go
...
func TestRouterGroup(t *testing.T) {
	engine := gosh.NewEngine()    // 假设您有一个创建 Engine 的方法
	group := engine.Group("/api") // 创建一个路由组

	// 注册一个简单的处理程序
	group.GET("/hello", func(c *gosh.Context) error {
...
func TestRouterGroupMiddleware(t *testing.T) {
	...
	// 注册中间件
	group.Use(func(c *gosh.Context) error {
	...
	// 注册处理程序
	group.GET("/test", func(c *gosh.Context) error {
...
func TestRouterGroup_ConflictHandling(t *testing.T) {
	engine := gosh.NewEngine()
	group := &gosh.RouterGroup{Engine: engine}

	// 定义一个简单的处理函数
	handler1 := func(ctx *gosh.Context) error {
		// 处理逻辑
		ctx.ResponseWriter.Write([]byte("Hello, Handler1"))
		return nil // 或者返回一个错误
	}

	handler2 := func(ctx *gosh.Context) error {
		// 处理逻辑
