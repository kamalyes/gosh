 ...
 * @FilePath: \gosh\tests\status_code_test.go
...
func TestSetAndGetStatusCode(t *testing.T) {
	// 定义一个自定义状态码
	const customCode gosh.StatusCode = 2001
	customMsg := "Custom Success"

	// 设置自定义状态码
	gosh.SetStatusCode(customCode, customMsg)

	// 获取自定义状态码对应的消息
	got := gosh.GetStatusCodeText(customCode)
	if got != customMsg {
...
func TestInitStatusCodes(t *testing.T) {
	// 定义自定义状态码和消息
	customCodes := map[gosh.StatusCode]string{
		200:  "Custom Success",
		2002: "Custom Error",
	...
	// 初始化自定义状态码
	gosh.InitStatusCodes(customCodes)

	// 验证自定义状态码的消息
	for code, expectedMsg := range customCodes {
		got := gosh.GetStatusCodeText(code)
		if got != expectedMsg {
...
func TestGetUnknownStatusCode(t *testing.T) {
	unknownCode := gosh.StatusCode(9999) // 一个未定义的状态码

	// 获取未知状态码的消息
	got := gosh.GetStatusCodeText(unknownCode)
	if got != "" {
